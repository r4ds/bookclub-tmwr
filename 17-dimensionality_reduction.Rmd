...
In this section we see four dimentionality reduction techniques:
...
knitr::include_graphics("images/17-pls.png")
```
...
```{r echo=FALSE, fig.align='center', fig.cap='[credits: PARTIAL LEAST-SQUARES REGRESSION: A TUTORIAL](http://home.mit.bme.hu/~horvath/IDA/1-s2.0-0003267086800289-main.pdf)', out.width='100%'}
knitr::include_graphics("images/17-pls_algo.png")
```
...
Save the models in a `.rda` file to save time.
    ...
    saveRDS(bean_res,"data/bean_res.RDS")
...
We do not actually read this back in these notes as the `.rda` file is too big for loading it, but you can use these commands on your machine:

    bean_res <- readRDS(file = "data/bean_res.rda")
    bean_res
...
knitr::include_graphics("images/17-workflow.png")
```
...
Save the `rankings` results as .RDS file and then read it back, with these commands:

    saveRDS(rankings,"data/rankings.RDS")
    rankings <- readRDS(file ="data/rankings.RDS")


Select the information you want to see:

```{r metricselection, eval=FALSE, include=T}
tidymodels_prefer()
...
```{r metricselection_image,echo=FALSE, fig.align='center', fig.cap='bean_res: workflow', out.width='100%'}
knitr::include_graphics("images/17-metricselection_image.png")
```

Finally, see the reults of the metrics ranking:

```{r ranking_plot, eval=FALSE, include=T, out.width='100%'}
rankings %>% 
...
knitr::include_graphics("images/17-model_ranks.png")
```
