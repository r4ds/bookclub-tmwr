...
Shame on you 😉 if you didn't guess I would make a scatter plot given a data set with two variables.
...
poly_fit3
#There are only two predictors in this model so this produces the same results as inter_fit but if there were 3 there would be three individual effects and the combination of those effects as interaction depending on if the third variable is continuous or categorical
```
...
reg_fits <-
  ...
  group_nest(group) %>% 
  mutate(
    fit = map(data, ~ lm(Volume ~ ., data = .x)),
    tidied = map(fit, tidy), #converts model object's coefficients to a dataframe
    glanced = map(fit, glance), #row summary of a model
    augmented = map(fit, augment) #returns a tibble of additional metrics like Cooks distance, lower and upper bounds of fitted values, standard errors of fitted values
  )
