...
[A Recommended preprocessing](A Recommended preprocessing)

```{r 08-model-step,echo=FALSE, fig.align='center', fig.cap='Models preprocessing steps',fig.width=10}
knitr::include_graphics("images/08-preproc.png")
```


```{r 08-libs,message=FALSE, warning=FALSE, paged.print=FALSE}
library(tidyverse)
library(tidymodels)
tidymodels_prefer()
```
...
```{r 08-wfl1, message=FALSE, warning=FALSE, paged.print=FALSE}
lm_model <- linear_reg() %>% 
  set_engine("lm")
...
```{r 08-fit1, message=FALSE, warning=FALSE, paged.print=FALSE}
lm_fit <- fit(lm_wflow, ames_train)
predict(lm_fit, ames_test %>% slice(1:3))
```

```{r 08-extract, message=FALSE, warning=FALSE, paged.print=FALSE}
lm_fit %>% 
  extract_recipe(estimated = TRUE)
```

```{r 08-extract_fit_parsnip, message=FALSE, warning=FALSE, paged.print=FALSE}
lm_fit %>% 
...
```{r 08-steps, message=FALSE, warning=FALSE, paged.print=FALSE}
simple_ames <- 
  recipe(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type,
         data = ames_train) %>%
  step_log(Gr_Liv_Area, base = 10) %>% 
...
```{r 08-plot1, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r 08-interaction, message=FALSE, warning=FALSE, paged.print=FALSE}
simple_ames <- 
  recipe(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type,
         data = ames_train) %>%
  step_log(Gr_Liv_Area, base = 10) %>% 
...
```{r 08-plot2, message=FALSE, warning=FALSE, paged.print=FALSE}
library(patchwork)
library(splines)
...
```{r 08-ns, message=FALSE, warning=FALSE, paged.print=FALSE}
...
> subsampling process should not be applied to the data being predicted.

skip that, when set to TRUE, will be ignored by the predict()
...
```{r 08-tidy, message=FALSE, warning=FALSE, paged.print=FALSE}
ames_rec <- 
  recipe(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type + 
...
we add the `id = "my_id"` in the `step_other()`
```{r 08-tidy2, message=FALSE, warning=FALSE, paged.print=FALSE}
ames_rec <- 
  recipe(Sale_Price ~ Neighborhood + Gr_Liv_Area + Year_Built + Bldg_Type + 
           Latitude + Longitude, data = ames_train) %>%
  step_log(Gr_Liv_Area, base = 10) %>% 
...
```{r 08-tidy3, message=FALSE, warning=FALSE, paged.print=FALSE}
estimated_recipe <- 
  lm_fit %>% 
  extract_recipe(estimated = TRUE)
...
```{r 08-tidy4, message=FALSE, warning=FALSE, paged.print=FALSE}
tidy(estimated_recipe, number = 2)
```

## COLUMN ROLES

add_role(), remove_role(), and update_role()
...
- [All possible recipes](https://www.tidymodels.org/find/)

- [Develop recipes](https://www.tidymodels.org/learn/develop/recipes/)
...
`r knitr::include_url("https://www.youtube.com/embed/ecOWidSqsAQ")`
