...
tidy(pull_workflow_prepped_recipe(fitted_workflow_lm), 3)
tidy(pull_workflow_fit(fitted_workflow_lm))
```
...
- If a modeling package uses the formula not only for preprocessing or has a syntax not supported by `model.matrix` you can specify a formula in `add_formula`
